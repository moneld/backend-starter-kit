...
export class PermissionsService {
     ...
     */
    async findById(id: string, includeRoles: boolean = false): Promise<Permission | null> {
     ...
     */
    async findByName(name: string, includeRoles: boolean = false): Promise<Permission | null> {
        return this.prisma.permission.findUnique({
            where: { name },
            include: {
                rolePermissions: includeRoles ? {
    ...
    async delete(id: string): Promise<Permission> {
        ...
        const systemPermissions = [
            ...
            'manage:roles', 'access:admin'
        ];

        if (systemPermissions.includes(existingPermission.name)) {
            throw new ConflictException(`Cannot delete system permission: ${existingPermission.name}`);
        }
    ...
    async getPermissionRoles(permissionId: string) {
        ...
        if (!permissionWithRoles.rolePermissions || !Array.isArray(permissionWithRoles.rolePermissions)) {
            return [];
        }
