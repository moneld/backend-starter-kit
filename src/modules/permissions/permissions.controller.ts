...
export class PermissionsController {
    ...
    @ApiResponse({ status: 200, description: 'Liste des permissions récupérée avec succès', type: [PermissionResponseDto] })
    @ApiResponse({ status: 401, description: 'Non authentifié' })
    @ApiResponse({ status: 403, description: 'Autorisation insuffisante' })
    async findAll(@Query('includeRoles') includeRoles?: boolean): Promise<PermissionResponseDto[]> {
        try {
            const permissions = await this.permissionsService.findAll(includeRoles === true);
    ...
    @ApiResponse({ status: 200, description: 'Permission récupérée avec succès', type: PermissionResponseDto })
    @ApiResponse({ status: 401, description: 'Non authentifié' })
    @ApiResponse({ status: 403, description: 'Autorisation insuffisante' })
    ...
    async findOne(
        ...
        try {
            const permission = await this.permissionsService.findById(id, includeRoles === true);
    ...
    @ApiResponse({ status: 201, description: 'Permission créée avec succès', type: PermissionResponseDto })
    ...
    @ApiParam({ name: 'id', description: 'ID de la permission' })
    @ApiResponse({ status: 200, description: 'Permission mise à jour avec succès', type: PermissionResponseDto })
    @ApiResponse({ status: 400, description: 'Données invalides' })
    @ApiResponse({ status: 401, description: 'Non authentifié' })
    @ApiResponse({ status: 403, description: 'Autorisation insuffisante' })
    @ApiResponse({ status: 404, description: 'Permission non trouvée' })
    ...
    @ApiResponse({ status: 403, description: 'Autorisation insuffisante' })
    @ApiResponse({ status: 404, description: 'Permission non trouvée' })
    @ApiResponse({ status: 409, description: 'Impossible de supprimer une permission système' })
    ...
    @ApiParam({ name: 'id', description: 'ID de la permission' })
    @ApiResponse({ status: 200, description: 'Rôles récupérés avec succès', type: [RoleMinimalDto] })
    @ApiResponse({ status: 401, description: 'Non authentifié' })
    @ApiResponse({ status: 403, description: 'Autorisation insuffisante' })
    @ApiResponse({ status: 404, description: 'Permission non trouvée' })
    async getPermissionRoles(@Param('id') id: string): Promise<RoleMinimalDto[]> {
        try {
            ...
            this.logger.error(`Failed to retrieve roles for permission with ID ${id}: ${error.message}`);
            throw error;
        }
    }
}