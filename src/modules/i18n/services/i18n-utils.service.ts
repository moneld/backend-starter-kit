...
export class I18nUtilsService {
  ...
  getLangFromRequest(request: FastifyRequest): string {
    ...
    const defaultLang = this.configService.get<string>(
      'app.defaultLanguage',
      'fr',
    );
  ...
  translate(key: string, args?: Record<string, any>, lang?: string): string {
    try {
      return this.i18nService.translate(key, {
        lang:
          lang || this.configService.get<string>('app.defaultLanguage', 'fr'),
        args,
      });
    } catch (error) {
  ...
  translateToAllLanguages(
    ...
    const supportedLanguages = this.configService.get<string[]>(
