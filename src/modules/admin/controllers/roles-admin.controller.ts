...
export class RolesAdminController {
    ...
    @Get('with-user-count')
    @ApiOperation({ summary: 'Obtenir tous les rôles avec le nombre d\'utilisateurs' })
    @ApiResponse({ status: 200, description: 'Rôles avec comptage utilisateurs récupérés avec succès' })
    @ApiResponse({ status: 401, description: 'Non authentifié' })
    @ApiResponse({ status: 403, description: 'Accès non autorisé' })
    ...
    @Get(':id/users')
    @ApiOperation({ summary: 'Obtenir les utilisateurs ayant un rôle spécifique' })
