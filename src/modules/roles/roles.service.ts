...
export class RolesService {
     ...
     */
    async findById(id: string, includePermissions: boolean = false): Promise<Role | null> {
     ...
     */
    async findByName(name: string, includePermissions: boolean = false): Promise<Role | null> {
        return this.prisma.role.findUnique({
            where: { name },
            include: {
                rolePermissions: includePermissions ? {
    ...
    async delete(id: string): Promise<Role> {
        ...
        if (['admin', 'user', 'moderator'].includes(existingRole.name)) {
            throw new ConflictException(`Cannot delete system role: ${existingRole.name}`);
        }
    ...
    async addPermission(roleId: string, permissionId: string): Promise<Role> {
        ...
        if (existingRolePermission) {
            throw new ConflictException(`Role already has permission: ${permission.name}`);
        }
    ...
    async removePermission(roleId: string, permissionId: string): Promise<Role> {
        ...
        try {
            ...
            throw new ConflictException(`Role does not have permission: ${permission.name}`);
        }
    ...
    async getRolePermissions(roleId: string) {
        ...
        if (!roleWithPerm.rolePermissions || !Array.isArray(roleWithPerm.rolePermissions)) {
            return [];
        }
