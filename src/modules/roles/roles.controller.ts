...
import { CreateRoleDto, PermissionDto, RoleResponseDto, RoleWithUsersDto, UpdateRoleDto } from './dto/role.dto';
...
export class RolesController {
    ...
    @ApiResponse({ status: 200, description: 'Liste des rôles récupérée avec succès', type: [RoleResponseDto] })
    @ApiResponse({ status: 401, description: 'Non authentifié' })
    @ApiResponse({ status: 403, description: 'Autorisation insuffisante' })
    async findAll(@Query('includePermissions') includePermissions?: boolean): Promise<RoleResponseDto[]> {
        try {
            const roles = await this.rolesService.findAll(includePermissions === true);

            return roles.map(role => {
                // Utiliser un cast explicite
                const roleWithPerm = role as any;
                const permissions = roleWithPerm.rolePermissions?.map((rp: any) => ({
                    id: rp.permission.id,
    ...
    @Get('with-user-count')
    @ApiOperation({ summary: 'Récupérer tous les rôles avec le nombre d\'utilisateurs' })
    @ApiResponse({ status: 200, description: 'Liste des rôles avec comptage utilisateurs récupérée avec succès', type: [RoleWithUsersDto] })
    @ApiResponse({ status: 401, description: 'Non authentifié' })
    @ApiResponse({ status: 403, description: 'Autorisation insuffisante' })
    async getRolesWithUserCount(): Promise<RoleWithUsersDto[]> {
        try {
            ...
            this.logger.error(`Failed to retrieve roles with user count: ${error.message}`);
            throw error;
        }
    }
    ...
    @ApiResponse({ status: 200, description: 'Rôle récupéré avec succès', type: RoleResponseDto })
    @ApiResponse({ status: 401, description: 'Non authentifié' })
    @ApiResponse({ status: 403, description: 'Autorisation insuffisante' })
    ...
    async findOne(
        ...
        try {
            const role = await this.rolesService.findById(id, includePermissions === true);
            ...
            this.logger.error(`Failed to retrieve role with ID ${id}: ${error.message}`);
            throw error;
        }
    }
    ...
    @ApiResponse({ status: 201, description: 'Rôle créé avec succès', type: RoleResponseDto })
    ...
    @ApiParam({ name: 'id', description: 'ID du rôle' })
    @ApiResponse({ status: 200, description: 'Rôle mis à jour avec succès', type: RoleResponseDto })
    @ApiResponse({ status: 400, description: 'Données invalides' })
    @ApiResponse({ status: 401, description: 'Non authentifié' })
    @ApiResponse({ status: 403, description: 'Autorisation insuffisante' })
    @ApiResponse({ status: 404, description: 'Rôle non trouvé' })
    ...
    async update(
        ...
        try {
            ...
            this.logger.error(`Failed to update role with ID ${id}: ${error.message}`);
            throw error;
        }
    }
    ...
    @ApiResponse({ status: 403, description: 'Autorisation insuffisante' })
    @ApiResponse({ status: 404, description: 'Rôle non trouvé' })
    @ApiResponse({ status: 409, description: 'Impossible de supprimer un rôle système' })
    async remove(@Param('id') id: string): Promise<void> {
        try {
            await this.rolesService.delete(id);
        } catch (error) {
            this.logger.error(`Failed to delete role with ID ${id}: ${error.message}`);
            throw error;
        }
    }
    ...
    @ApiParam({ name: 'id', description: 'ID du rôle' })
    @ApiResponse({ status: 200, description: 'Permissions récupérées avec succès', type: [PermissionDto] })
    @ApiResponse({ status: 401, description: 'Non authentifié' })
    @ApiResponse({ status: 403, description: 'Autorisation insuffisante' })
    @ApiResponse({ status: 404, description: 'Rôle non trouvé' })
    async getRolePermissions(@Param('id') id: string): Promise<PermissionDto[]> {
        try {
            ...
            this.logger.error(`Failed to retrieve permissions for role with ID ${id}: ${error.message}`);
            throw error;
        }
    }
    ...
    @ApiResponse({ status: 200, description: 'Permission ajoutée avec succès', type: RoleResponseDto })
    @ApiResponse({ status: 401, description: 'Non authentifié' })
    @ApiResponse({ status: 403, description: 'Autorisation insuffisante' })
    ...
    async addPermissionToRole(
        ...
        try {
            ...
            this.logger.error(`Failed to add permission ${permissionId} to role ${id}: ${error.message}`);
            throw error;
        }
    }
    ...
    @ApiParam({ name: 'id', description: 'ID du rôle' })
    @ApiParam({ name: 'permissionId', description: 'ID de la permission' })
    @ApiResponse({ status: 200, description: 'Permission supprimée avec succès', type: RoleResponseDto })
    @ApiResponse({ status: 401, description: 'Non authentifié' })
    @ApiResponse({ status: 403, description: 'Autorisation insuffisante' })
    @ApiResponse({ status: 404, description: 'Rôle ou permission non trouvé' })
    ...
    async removePermissionFromRole(
        ...
        try {
            ...
            this.logger.error(`Failed to remove permission ${permissionId} from role ${id}: ${error.message}`);
            throw error;
        }
    }

    @Get(':id/users')
    @ApiOperation({ summary: 'Récupérer les utilisateurs ayant un rôle spécifique' })
    @ApiParam({ name: 'id', description: 'ID du rôle' })
    ...
    async getRoleUsers(@Param('id') id: string): Promise<any[]> {
        try {
            ...
            this.logger.error(`Failed to retrieve users for role with ID ${id}: ${error.message}`);
            throw error;
        }
    }
}